...
server.on('message', function (msg, rinfo) {
    ...
    try {
        ...
        if (zone && question_1) {
            ...
            var authorities = [];
            if (record) {
                if (record && question_1.type == "CNAME" || record.type == "CNAME") {
                ...
                else if (record && question_1.type == "A" || record.type == "A") {
                    answers.push(getA_Record(record, question_1.name));
                }
                else if (record && question_1.type == "SOA" || record.type == "SOA") {
                    answers.push(getSOA_Record(zone));
                }
                else if (record && question_1.type == "NS" || record.type == "NS") {
...
server.on("listening", function () {
    console.log("UDP - DNS Server running on ".concat(PORT, "..."));
});
